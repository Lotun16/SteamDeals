...
import { useItadPrices, useItadHistoricalLows, useItadPriceHistory } from "../hooks/useItad";
...
export const GameDetails: VFC<GameDetailsProps> = ({ gameId, gameTitle }) => {
	...
	const { data: priceHistory, loading: historyLoading, error: historyError } = useItadPriceHistory(gameId, STEAM_SHOPS);

	const loading = pricesLoading || lowsLoading || historyLoading;
	const error = pricesError || lowsError || historyError;

	const processedData = useMemo(() => {
		if (loading || error || !historicalLows || !priceHistory) return null;
		...
		<div style={{ marginTop: "10px" }}>
			...
			{!viewChart ? (
				<div>
					<div style={{ display: "flex", gap: "25px", padding: "25px 30px", height: "200px" }}>
						...
						<div style={{ display: "flex", flexDirection: "column", justifyContent: "space-evenly" }}>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
								<div>
									Original Price: {processedData?.currency} {processedData?.prices.original}
								</div>
								<div>
									Current Price:
									<div>
								...
								<div>
									Historical Low:
									<div>
										<GamePrice currency={processedData?.currency} price={processedData?.prices?.lowest} cut={processedData?.prices?.lowestCut ? processedData?.prices?.lowestCut : undefined} />
										{processedData?.prices.lowest === processedData?.prices.current ? <HighlightText variant="buy">BUY</HighlightText> : <HighlightText variant="wait">WAIT</HighlightText>}
									</div>
								</div>
							</div>
						</div>
					...
					<div>
						<h2>Price History</h2>
						<div>{processedData.priceHistory && <GamePriceHistory priceHistory={processedData.priceHistory} />}</div>
					</div>
				</div>
			) : (
				<div>
